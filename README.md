...
Two scripts, `wandb_demo.py` and `dagshub_demo.py`, demonstrate the tracking integrations. Both generate synthetic metric curves, cosine interpolation with noise, and log them through the real W&B and MLflow client APIs. No model is trained and no dataset is loaded, the point is the logging plumbing, the sweep structure, and the dashboard comparison workflow, not the numbers. Every run they create is tagged synthetic-demo so the curves cannot be read as measurements.

```bash
pip install wandb dagshub mlflow
python wandb_demo.py
python dagshub_demo.py
```

Without accounts on those services both scripts fall back to local logging, and the MLflow side can be viewed with `mlflow ui --port 5000`.

...
================================================
Simulates 3 training runs comparing learning rates for the mlops-training-pipeline project.
Pushes realistic metrics to Weights & Biases for public portfolio visibility.
...
def run_experiment(exp: dict, project: str, entity: str) -> None:
    run = wandb.init(
        ...
        name=exp["name"],
        config=exp["config"],
        tags=["lr-comparison", "resnet18", "cifar10"],
        notes=(
            f"Learning rate sweep: {exp['config']['learning_rate']:.0e}. "
            "Part of a 3-run comparison to identify optimal LR for ResNet-18 on CIFAR-10."
        ),
    )
